...
class VideoOrderController extends AdminController
{
    ...
    public function grid()
    {
        ...
        return Grid::make($Order, function (Grid $grid) use ($Order) {
            ...
            /* 导出 */
            $grid->export($this->columns_name)->rows(function (array $rows) {
                foreach ($rows as &$row) {
    ...
    {
        $Order = new OrderVideo(['orders']);
        return Show::make($id, $Order, function (Show $show) {
            $show->field('order_id');
            $show->field('user_id');
            $show->field('order_no');
            $show->field('name');
    ...
    {
        $Order = new OrderVideo(['orders']);
        return Form::make($Order, function (Form $form) {
            $form->display('id');
            $form->field('order_id');
            $form->field('user_id');
            $form->field('order_no');
